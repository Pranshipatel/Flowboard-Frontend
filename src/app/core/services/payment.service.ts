...
export class PaymentService {
  ...
  getSubscriptionStatus(userId: number): Observable<any> {
    return this.http.get<any>(`${environment.apiBaseUrl}/subscription/status/${userId}`).pipe(
      tap(response => {
        // Backend returns { userId, plan: "PREMIUM", status: "ACTIVE" }
        // Map `plan` → `planName` so components using currentPlan() work correctly
        const normalized = {
          ...response,
          planName:        response.planName ?? response.plan ?? 'FREE',
          planDisplayName: response.planDisplayName ?? response.plan ?? 'Free',
          status:          response.status ?? 'EXPIRED',
        };

        this.currentPlan.set(normalized);

        const isPremium = normalized.planName === 'PREMIUM' && normalized.status === 'ACTIVE';
        this.subscriptionStatus.next(isPremium ? 'PREMIUM' : 'FREE');
      }),
      catchError(err => {
